...
wire [31:0] imem_dout;
instr_mem #(.ADDR_WIDTH(13), .DATA_WIDTH(32)) (.clk(clk), .we(1'b0), .addr(imem_addr), 
...
wire mux_pc;
wire mux_jalr;
control_unit(.clk(clk), .rst(rst), .opcode(imem_dout[6:0]), .funct3(imem_dout[14:12]), .funct7(imem_dout[30]), 
...
wire [31:0] immediate;
imm_gen(.instruction(imem_dout), .immSel(immSel), .imm(immediate));

//Instantiate register file
wire [31:0] reg_write_data;
wire [31:0] reg_data_1;
wire [31:0] reg_data_2;
reg_file(.clk(clk), .rst(rst), .we(we_rf), .read_reg_1(imem_dout[24:20]), .read_reg_2(imem_dout[19:15]), 
...
alu(.in_1(alu_oper_1), .in_2(alu_oper_2), .control(aluop), .out(alu_out_pre));
always@(posedge clk)
begin
    if(we_result)
...
data_mem #(.ADDR_WIDTH(13), .DATA_WIDTH(32)) (.clk(clk), .we(we_dmem), .addr(alu_out_reg[14:2]), //Double check this
